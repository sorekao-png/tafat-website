...
describe("SEO production host guard", () => {
  test("sitemap.xml contains exactly the three expected tafat.co.uk locs", () => {
    const xml = read("public/sitemap.xml");
    const locs = [...xml.matchAll(/<loc>(.*?)<\/loc>/g)].map((m) => m[1]);
    expect(locs).toEqual([`${SITE}/`, `${SITE}/privacy`, `${SITE}/terms`]);
  });
  ...
  test("canonical SEO metadata in source routes uses the production host", () => {
    ...
    const all = [root, privacy, terms].join("\n");
