    ...
    <main>
      ...
      <section id={EDITORIAL_SECTION_ID} className="editorial-standard" aria-labelledby="editorial-standard-title">
        <div className="wrap">
          ...
          <h2 id="editorial-standard-title">{EDITORIAL_TITLE}</h2>
          <p className="es-subtitle">{EDITORIAL_SUBTITLE}</p>
          <p className="es-intro">{EDITORIAL_INTRO}</p>
          <h3 className="es-principles-title">{PRINCIPLES_TITLE}</h3>
          <div className="principle-grid">
            {EDITORIAL_PRINCIPLES.map((p) => {const Icon = principleIconByKey[p.art];return <article className="principle-card" key={p.name}><span className="principle-icon" aria-hidden="true"><Icon /></span><h4>{p.name}</h4><p>{p.copy}</p></article>;})}
          </div>
          <div className="es-note"><span className="es-note-icon" aria-hidden="true"><ShieldIcon /></span><div><h3>{IMPORTANT_NOTE_TITLE}</h3><p>{IMPORTANT_NOTE_COPY}</p></div></div>
          ...
          <div className="es-actions"><a className="primary-button" href={EDITORIAL_STANDARDS_PATH}>{LEARN_MORE_LABEL} <span aria-hidden="true">→</span></a></div>
          <footer className="es-integrity"><strong>{INTEGRITY_TITLE}</strong><p>{INTEGRITY_DISCLOSURE}</p><p className="es-loyalty">{INTEGRITY_LOYALTY}</p></footer>
        </div>
      </section>
