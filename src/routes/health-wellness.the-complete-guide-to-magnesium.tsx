...
type MagnesiumProduct = {
  label: string; category: string; name: string; form: string; description: string;
  investigate: string[]; consideration: string; image: string; alt: string;
  facts: string; elemental: string; format: string; other: string; href: string;
};
const magnesiumProducts: MagnesiumProduct[] = [
  { label: "TAFAT FORMULATION CANDIDATE", category: "Magnesium complex", name: "Triple Magnesium Complex", form: "Citrate · Malate · Oxide", description: "A three-form magnesium complex presented in softgels.", investigate: ["A reader comparing a multi-form magnesium complex", "Someone checking how a compound amount is presented on a label"], consideration: "The 600 mg is a complex amount; do not treat it as elemental magnesium.", image: "/magnesium-products/triple.png", alt: "Triple Magnesium Softgels, Citrate Malate & Oxide, Highly Absorbable, 60 Ct | Magnesium Supplement for Digestion, Calm, Relaxation, Leg Cramps & Sleep Support, 600mg Complex, Easy to Swallow, 60 Softgels", facts: "600 mg complex · 60 softgels", elemental: "Not stated here; verify current label", format: "Softgels", other: "Citrate, malate and oxide", href: "https://lasso.to/amazon/7HfrxQuc4K" },
  { label: "TAFAT FORMULATION CANDIDATE", category: "Magnesium citrate", name: "Magnesium Citrate Gummies", form: "Citrate", description: "A citrate formulation delivered as vegan gummies.", investigate: ["A reader comparing a chewable delivery format", "Someone checking whether citrate suits their intended use"], consideration: "Elemental magnesium status should be verified on the current Supplement Facts panel.", image: "/magnesium-products/citrate.png", alt: "Magnesium Citrate Gummies, 200 mg per Serving, 60 Vegan Gummies", facts: "200 mg per serving · 60 vegan gummies", elemental: "Verify current label", format: "Vegan gummies", other: "Verify current label", href: "https://lasso.to/amazon/JkM4ekPyZ1" },
  { label: "TAFAT FORMULATION CANDIDATE", category: "Magnesium glycinate", name: "Magnesium Glycinate Gummies", form: "Glycinate", description: "A glycinate formulation with added L-theanine and lemon balm.", investigate: ["A reader comparing a gummy glycinate formulation", "Someone reviewing a magnesium product with additional active ingredients"], consideration: "Verify the current Supplement Facts panel, including the magnesium amount and added ingredients.", image: "/magnesium-products/glycinate.png", alt: "Magnesium Glycinate Gummies with L-theanine and lemon balm, 60 Count", facts: "60 count · Added L-theanine and lemon balm", elemental: "Verify current label", format: "Gummies", other: "L-theanine and lemon balm", href: "https://lasso.to/amazon/FcRI4wUlHI" },
  { label: "TAFAT FORMULATION CANDIDATE", category: "Mineral combination", name: "Calcium, Magnesium, Zinc & D3", form: "Combination formulation", description: "A combination mineral formulation that also includes vitamin D3.", investigate: ["A reader comparing a multi-nutrient mineral product", "Someone checking several nutrients in one serving"], consideration: "Verify current label for amounts.", image: "/magnesium-products/calcium.png", alt: "Calcium Magnesium Zinc and Vitamin D3 Supplement, 90 Tablets", facts: "Verify current label for amounts", elemental: "Verify current label", format: "Tablets", other: "Calcium, zinc and vitamin D3", href: "https://lasso.to/amazon/7WxyavBqnH" },
  { label: "TAFAT FORMULATION CANDIDATE", category: "Magnesium taurate", name: "Magnesium Taurate", form: "Taurate", description: "A taurate formulation presented with vitamin B6 and BioPerine.", investigate: ["A reader comparing a taurate formulation", "Someone checking compound and elemental amounts together"], consideration: "Check the current label for serving size and formulation details.", image: "/magnesium-products/taurate.png", alt: "Magnesium Taurate Capsules, 1,900 mg Compound, 150 mg Elemental Magnesium per Serving, with Vitamin B6 and BioPerine, 60 Capsules", facts: "1,900 mg compound · 150 mg elemental per serving · B6 + BioPerine", elemental: "150 mg per serving", format: "Capsules", other: "Vitamin B6 and BioPerine", href: "https://lasso.to/amazon/o2FXVEK7Gq" },
  { label: "TAFAT FORMULATION CANDIDATE", category: "Magnesium malate", name: "Magnesium Malate", form: "Malate", description: "A malate formulation presented with vitamin B6 and BioPerine.", investigate: ["A reader comparing a malate formulation", "Someone checking compound and elemental amounts together"], consideration: "Check the current label for serving size and formulation details.", image: "/magnesium-products/malate.png", alt: "Magnesium Malate Capsules, 1,500 mg Compound, 300 mg Elemental Magnesium per Serving, with Vitamin B6 and BioPerine, 90 Capsules", facts: "1,500 mg compound · 300 mg elemental per serving · B6 + BioPerine", elemental: "300 mg per serving", format: "Capsules", other: "Vitamin B6 and BioPerine", href: "https://lasso.to/amazon/1IAxsFOjIP" },
];
...
function MagnesiumComparison() {
  return <section className="magnesium-comparison" id="magnesium-formulation-candidates">
    <h2>TAFAT MAGNESIUM FORMULATION GUIDE</h2>
    <p className="comparison-subheading">Six formulation candidates to investigate before you buy.</p>
    <p>This educational comparison brings six verified retailer listings together so readers can inspect formulation, delivery format and label details in one place. These are candidates for investigation, not rankings or winners. Always check the current retailer page and Supplement Facts.</p>
    <p className="comparison-disclosure"><strong>Affiliate disclosure:</strong> Some links below are affiliate links. If you buy through one, TAFAT may earn a commission at no additional cost to you. TAFAT has not independently evaluated these products.</p>
    <div className="callout warning"><strong>COMPOUND WEIGHT IS NOT ALWAYS ELEMENTAL MAGNESIUM</strong><p>A label may show the weight of the full magnesium compound rather than the amount of elemental magnesium. These numbers are not interchangeable. Check the Supplement Facts panel for elemental magnesium, serving size and the current amount before comparing products.</p></div>
    <div className="magnesium-product-grid">{magnesiumProducts.map((product) => <article className="magnesium-product-card" key={product.name}>
      ...
      <p className="magnesium-product-label">{product.label} · {product.category}</p><h3>{product.name}</h3><p className="magnesium-product-form">{product.form}</p><p>{product.description}</p><p><strong>Why someone may investigate it</strong></p><ul>{product.investigate.map((item) => <li key={item}>{item}</li>)}</ul><p><strong>Important consideration</strong>: {product.consideration}</p>
      <a href={product.href} target="_blank" rel="sponsored nofollow noopener" onClick={() => trackEvent("magnesium_product_click", { product_name: product.name, magnesium_form: product.form, article_slug: "the-complete-guide-to-magnesium", affiliate_network: "Lasso" })}>View Product &amp; Current Price</a>
    </article>)}</div>
    <div className="magnesium-comparison-table-wrap"><table><caption>At-a-glance formulation details</caption><thead><tr><th>Product</th><th>Primary magnesium form</th><th>Delivery format</th><th>Other active ingredients</th><th>Elemental magnesium</th><th>Best suited to investigate if...</th><th>Important consideration</th></tr></thead><tbody>{magnesiumProducts.map((p) => <tr key={p.name}><th scope="row">{p.name}</th><td>{p.form}</td><td>{p.format}</td><td>{p.other}</td><td>{p.elemental}</td><td>{p.investigate[0]}</td><td>{p.consideration}</td></tr>)}</tbody></table></div>
